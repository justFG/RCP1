...
const Navbar = ({ hp, pr,ab }) => {
  ...
  return (
    <div className="z-50 relative">
      <nav>
        <div className={`hidden md:flex h-18 items-center text-l gap-3 pl-15 pr-15 justify-between shadow ${hp ? "bg-[#001433]" : "bg-[#F5F5F5]"}`}>
          <img className="cursor-pointer" src={hp ? "Logo.png" : "LogoC.png"} width='13%' onClick={homepagenav}/>
      ...
      <nav>
        <div className={`flex md:hidden h-18 items-center text-l gap-3 justify-between shadow ${hp ? "bg-[#001433]" : "bg-[#F5F5F5]"}`}>
          <img src={hp ? "Logo.png" : "LogoC.png"} width='50%' onClick={homepagenav}/>
          <a className={`mr-6 transition-transform duration-30 ${hp ? "text-white" : "text-[#000000]"} ${Menu ? "rotate-90" : "rotate-0"}`} onClick={() => setMenu(!Menu)}><MenuIcon/></a>
        </div>
